...
- [x] Check [inboxes](./week-of-22/inbox-list.md)
- [x] write stand up
...
#### data model
- make it so it's not possible for a node to exist but not the corresponding relation lists (e.g. by denormalizing: make it a real property of the node) (maybe it's not even it's own class, just an object. easier to do once fractional position is just a string)
