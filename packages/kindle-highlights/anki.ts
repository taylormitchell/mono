...
async function anki(action: string, params: any): Promise<AnkiResponse> {
...
async function insertFlashcard(note: AnkiNote): Promise<void> {
...
function getKindleUrl(asin: string, location: string): string {
...
export async function ankifyRecent(): Promise<void> {
  ...
  let highlights: Highlight[] = await fetch("https://readwise.io/api/v2/highlights/", {
    method: "GET",
    headers: {
  ...
  let books: Book[] = await fetch("https://readwise.io/api/v2/books/", {
    method: "GET",
    headers: {
      Authorization: `Token ${process.env.READWISE_API_KEY}`,
    },
